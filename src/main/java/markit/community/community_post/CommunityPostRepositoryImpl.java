...
import markit.community.community_category.QCommunityCategory;
import markit.community.community_topic.QCommunityTopic;
...
public class CommunityPostRepositoryImpl implements CommunityPostRepositoryCustom {
    ...
    @Override
    public Page<CommunityPost> findBySearchOption(
            Pageable pageable,
        ...
        // 데이터 조회 쿼리
        List<CommunityPost> posts = queryFactory.select(post)
                .distinct()
                .from(post)
                .leftJoin(
                        post.topic,
        ...
        // 전체 카운트 쿼리(페이지네이션용)
        Long total = queryFactory.select(post.countDistinct())
                .from(post)
                .leftJoin(
                        post.topic,
                        topic
