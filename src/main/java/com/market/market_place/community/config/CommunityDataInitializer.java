package com.market.market_place.community.config;
...
public class CommunityDataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        // 3. 회원 로딩
        Map<Long, Member> users = loadMembers(2L, 3L); // member_id 2, 3

        // 4. 게시글 생성
        CommunityPost p1 = createPostIfAbsent(users.get(2L), t1, "우리 동네 맛집 추천해요", "어제 다녀온 고기집 진짜 맛있었어요! 추천드려요.", "서울 강남구", 1, 1);
        CommunityPost p2 = createPostIfAbsent(users.get(3L), t2, "편의점 신상 나왔네요", "우리 동네 CU에 신상 아이스크림 들어왔어요.", "서울 마포구", 1, 1);
        CommunityPost p3 = createPostIfAbsent(users.get(2L), t3, "근처 약국 추천 부탁드려요", "감기약 살 수 있는 약국 추천해주세요.", "서울 서대문구", 1, 1);
        CommunityPost p4 = createPostIfAbsent(users.get(3L), t4, "가성비 좋은 미용실 아시나요?", "머리 커트 잘하는 곳 찾고 있어요.", "서울 동작구", 1, 1);
        CommunityPost p5 = createPostIfAbsent(users.get(2L), t5, "강아지 산책 모임 해요", "주말에 반려견 산책 같이하실 분 계신가요?", "서울 송파구", 1, 1);
        CommunityPost p6 = createPostIfAbsent(users.get(3L), t6, "헬스장 추천 좀요", "PT 잘 봐주는 헬스장 있나요?", "서울 용산구", 1, 1);
        CommunityPost p7 = createPostIfAbsent(users.get(2L), t7, "취미로 그림 그리시는 분?", "취미로 수채화 그리는데 같이 해요.", "서울 성동구", 1, 1);
        CommunityPost p8 = createPostIfAbsent(users.get(3L), t8, "진로 고민 나눠요", "취업 준비 중인데 고민이 많네요.", "서울 종로구", 1, 1);
        CommunityPost p9 = createPostIfAbsent(users.get(2L), t9, "이번 주말 플리마켓 열린대요", "동네 공원에서 플리마켓 한다고 해요.", "서울 강서구", 1, 1);
        CommunityPost p10 = createPostIfAbsent(users.get(3L), t10, "분실물 찾아요", "어제 버스에서 가방을 잃어버렸습니다.", "서울 관악구", 1, 1);

        // 5. 댓글 생성
        createCommentIfAbsent(p1, users.get(3L), "저도 거기 가봤는데 맛있더라구요!", null, 1);
        createCommentIfAbsent(p1, users.get(2L), "오호 저도 가봐야겠네요~", null, 1);

        createCommentIfAbsent(p2, users.get(2L), "저도 그 아이스크림 먹어봤어요!", null, 1);
        createCommentIfAbsent(p2, users.get(3L), "편의점 신상 정보 감사합니다 ^^", null, 1);

        createCommentIfAbsent(p3, users.get(3L), "근처 ○○약국 좋아요", null, 1);
        createCommentIfAbsent(p3, users.get(2L), "△△약국도 친절하더라구요", null, 1);

        createCommentIfAbsent(p4, users.get(2L), "저는 ○○헤어 추천드려요!", null, 1);
        createCommentIfAbsent(p4, users.get(3L), "저렴하고 잘하는 곳 있어요~ 쪽지 드릴게요.", null, 1);

        createCommentIfAbsent(p5, users.get(3L), "강아지랑 같이 산책하고 싶어요!", null, 1);
        createCommentIfAbsent(p5, users.get(2L), "저희 강아지도 참가해도 될까요?", null, 1);

        createCommentIfAbsent(p7, users.get(3L), "저도 그림 같이 그려보고 싶어요~", null, 1);
        createCommentIfAbsent(p7, users.get(2L), "좋아요! 수채화 모임 저도 참여할래요", null, 1);

        createCommentIfAbsent(p8, users.get(2L), "취업 준비 힘내세요! 저도 비슷한 상황이에요", null, 1);
        createCommentIfAbsent(p8, users.get(3L), "조언 필요하면 DM 주세요", null, 1);

        createCommentIfAbsent(p9, users.get(3L), "플리마켓 저도 가볼래요!", null, 1);
        createCommentIfAbsent(p9, users.get(2L), "주말에 가족과 함께 가야겠네요", null, 1);

        createCommentIfAbsent(p10, users.get(2L), "분실물 빨리 찾으시길 바래요", null, 1);
        createCommentIfAbsent(p10, users.get(3L), "혹시 CCTV 확인해보셨나요?", null, 1);
    }
    ...
    private CommunityComment createCommentIfAbsent(CommunityPost post, Member author, String content, String imageUrl, int likeCount) {
